...
const upgrade = async function(dir, name, pkg) {
  ...
  await download(projectDir, name, fileJSON(['yarn.lock', 'package.json']))

  //call yarn upgrade pkg in projectDir
  var {code, output} = await prosh(`yarn upgrade ${pkg}`, {cwd: projectDir})
  ...
  await upload(projectDir, name, fileJSON(['yarn.lock', 'package.json']))
